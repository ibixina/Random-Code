...
def process_billboard_csv(input_csv="./billboard.csv", output_csv="./billboard_year_end_2006_2024_word_counts.csv"):
    """
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            song_query = f"{song_name} {song_artist}"
            print(f"Searching for: {song_query}")
            
            # Get the API path and then the song URL from Genius
            song_api_path = get_song_id(song_query)
            if not song_api_path:
                print("Song not found in Genius API.")
                continue
            
            song_url = get_song_lyrics_url(song_api_path)
            if not song_url:
                print("Song URL not found or invalid.")
                continue
